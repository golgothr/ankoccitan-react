...
import { reviradaApi } from '../../../core/api/reviradaApi';
import { logger } from '@/core/utils/logger';

interface ReviradaApiKeyFormProps {
  onSuccess?: (message: string) => void;
  onError?: (message: string) => void;
}

export const ReviradaApiKeyForm: React.FC<ReviradaApiKeyFormProps> = ({
  onSuccess,
  onError,
}) => {
  ...
  const loadCurrentApiKey = useCallback(async () => {
    try {
      setIsLoading(true);
      const key = await settingsApi.getApiKey('revirada');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      await settingsApi.upsertApiKey('revirada', apiKey.trim());
  ...
  const handleDelete = async () => {
    ...
    if (
      !window.confirm(
        'Êtes-vous sûr de vouloir supprimer votre clé API Revirada ?'
      )
    ) {
      return;
    }
    ...
    try {
      setIsLoading(true);
      await settingsApi.deleteApiKey('revirada');
  ...
  const handleTestApiKey = async () => {
    ...
    try {
      setIsLoading(true);
      await reviradaApi.translate('bon dia');
      onSuccess?.('Clé API Revirada valide !');
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Entrez votre clé API Revirada"
            ...
            <div className="flex items-center justify-between mt-1">
              <p className="text-xs text-gray-500">
                ...
                <a
                  href="https://revirada.example.com/api/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-orange-600 hover:text-orange-700 underline"
                >
                  revirada.example.com/api
                </a>
              </p>
              <button
                type="button"
