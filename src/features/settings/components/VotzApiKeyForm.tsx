...
import { votzApi } from '../../../core/api/votzApi';
import { logger } from '@/core/utils/logger';

interface VotzApiKeyFormProps {
  onSuccess?: (message: string) => void;
  onError?: (message: string) => void;
}

export const VotzApiKeyForm: React.FC<VotzApiKeyFormProps> = ({
  onSuccess,
  onError,
}) => {
  ...
  const loadCurrentApiKey = useCallback(async () => {
    try {
      setIsLoading(true);
      const key = await settingsApi.getApiKey('votz');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      await settingsApi.upsertApiKey('votz', apiKey.trim());
  ...
  const handleDelete = async () => {
    ...
    if (
      !window.confirm('Êtes-vous sûr de vouloir supprimer votre clé API VOTZ ?')
    ) {
      return;
    }
    ...
    try {
      setIsLoading(true);
      await settingsApi.deleteApiKey('votz');
  ...
  const handleTestApiKey = async () => {
    ...
    try {
      setIsLoading(true);
      await votzApi.textToSpeech('bon dia');
      onSuccess?.('Clé API VOTZ valide !');
    } catch (error) {
  ...
  return (
    <div className="bg-white border rounded-lg p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">Clé API VOTZ</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Entrez votre clé API VOTZ"
            ...
            <div className="flex items-center justify-between mt-1">
              <p className="text-xs text-gray-500">
                ...
                <a
                  href="https://votz.example.com/api/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-orange-600 hover:text-orange-700 underline"
                >
                  votz.example.com/api
                </a>
              </p>
              <button
                type="button"
